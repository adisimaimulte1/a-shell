...
& $compiler -target x86_64-w64-mingw32 -std=c++23 -O2 -shared -static -DUNICODE -D_UNICODE -DWH_MOD -include windhawk_api.h (Join-Path $root 'assets\windhawk\ashell-lockscreen-clear-background.wh.cpp') $library -lcomctl32 -lole32 -loleaut32 -lruntimeobject -lversion '-Wl,--export-all-symbols' -o (Join-Path $root 'assets\windhawk\ashell-lockscreen-clear-background_1.7.dll')
if($LASTEXITCODE){throw 'Lock-screen styling mod compilation failed.'}
