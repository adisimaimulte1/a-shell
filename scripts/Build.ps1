...
& $Compiler -target x86_64-w64-mingw32 (Join-Path $root 'src\CursorSessionGuard.cpp') -o (Join-Path $root 'bin\CursorSessionGuard.exe') -std=c++17 -O2 -municode -mwindows -static -luser32 -lgdi32
if($LASTEXITCODE -ne 0){throw 'Cursor session guard build failed.'}
