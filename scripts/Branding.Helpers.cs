...
public static class AShellBranding {
 ...
 static byte[] Block(string name,byte[] value,bool text,params byte[][] children){
  using(var m=new MemoryStream())using(var w=new BinaryWriter(m)){
   w.Write((ushort)0);w.Write((ushort)(text?value.Length/2:value.Length));w.Write((ushort)(text?1:0));w.Write(Encoding.Unicode.GetBytes(name+"\0"));Pad(w);w.Write(value);Pad(w);
   foreach(var child in children){w.Write(child);Pad(w);}var b=m.ToArray();Array.Copy(BitConverter.GetBytes((ushort)b.Length),b,2);return b;
  }
 }
 ...
 public static void Apply(string exe,string png,string ico,string description){
  var sizes=new int[]{16,24,32,48,64,128,256};var images=new byte[sizes.Length][];
  using(var original=Image.FromFile(png))for(int i=0;i<sizes.Length;i++){
   int size=sizes[i];using(var bitmap=new Bitmap(size,size,PixelFormat.Format32bppArgb)){
    using(var g=Graphics.FromImage(bitmap)){g.Clear(Color.Transparent);g.InterpolationMode=InterpolationMode.HighQualityBicubic;float scale=Math.Min(size/(float)original.Width,size/(float)original.Height);float w=original.Width*scale,h=original.Height*scale;g.DrawImage(original,(size-w)/2,(size-h)/2,w,h);}
    using(var m=new MemoryStream()){bitmap.Save(m,ImageFormat.Png);images[i]=m.ToArray();}
   }
  }
  ...
  IntPtr handle=BeginUpdateResource(exe,false);if(handle==IntPtr.Zero)throw new IOException("Cannot open executable resources");
  bool done=false;try{
   using(var m=new MemoryStream())using(var w=new BinaryWriter(m)){
    ...
    for(int i=0;i<sizes.Length;i++){
     ...
     w.Write((byte)(sizes[i]==256?0:sizes[i]));w.Write((byte)(sizes[i]==256?0:sizes[i]));w.Write((ushort)0);w.Write((ushort)1);w.Write((ushort)32);w.Write(images[i].Length);w.Write((ushort)(i+1));
    }
    byte[] b=m.ToArray();if(!UpdateResource(handle,(IntPtr)14,(IntPtr)1,0,b,(uint)b.Length))throw new IOException("Icon group failed");
   }
   byte[] v=Version(exe,description);if(!UpdateResource(handle,(IntPtr)16,(IntPtr)1,0,v,(uint)v.Length))throw new IOException("Version resource failed");
   done=EndUpdateResource(handle,false);if(!done)throw new IOException("Saving resources failed");
