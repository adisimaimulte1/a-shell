...
Assert ($installer -match 'Upgrade / repair current A-Shell') 'Installer must advertise in-place upgrade/repair.'
Assert ($installer -match 'PreserveUserFiles') 'Installer must preserve recovery/user-owned data during upgrade.'
Assert ($installer -match 'StopExistingMatrix') 'Installer must stop the current Matrix process before replacing program files.'
